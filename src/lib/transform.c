...
int noteToList(int argc, char *argv[], LIST list)
{
    ...
    {
        if(sscanf(line, "$%d| %[^\t\n]", quantity, command)==2) {printf("%d %s\n", *quantity, command); list_add(list, *quantity, command, NULL);}//%[^\t\n]
        else if(sscanf(line, "$| %[^\t\n]", command)==1) {printf("%s\n", command); list_add(list, 1, command, NULL);}
        else if(sscanf(line, "$ %[^\t\n]", command)==1) {printf("%s\n", command); list_add(list, 0, command, NULL);}
